...
def main():
    ...
    sub = rospy.Subscriber(
        "/mocap_node/pelota/pose", PoseStamped, funcion
    )  # duda como puedo hacer que me devuelva cada x segundo y no continuo, y no se para que sirve la funcion
    ...
    wpose = arm_group.get_current_pose().pose  # guarda en w.pose la posicion actual del brazo
    posicion_brazo_x.append(
        arm_group.get_current_pose().pose.position.x
    )  # anado a la lista la posicion x de la mano
    posicion_brazo_y.append(
        arm_group.get_current_pose().pose.position.y
    )  # anado a la lista la posicion y de la mano

    while (
        x_actual > corx_mano + 0.25
    ):  # condicional mientras que la pelota no llega a la distancia 0,10 de la mano sigue entrando
        ...
        if x2 > x1:
            if (
                y2 != y1
            ):  # se tiene que hacer este if porque depende de si la posicion x2 es la mas alejada o la mas cercana.
                    ...
                    if x_actual < 1.65:  # filtro para que no empiexe
                        if y < 0.75:  # dato analitico fin de mesa
                            if y > -0.75:  # dato analitico fin de mesa
                                if (
                                    ite > 2
                                ):  # se realiza el if porque sino queda fuera del alcance
                                    while x_actual > corx_mano + 0.25:
                                        ...
                                        if x2 > x1:
                                            if (
                                                ...
                                                m = (x2 - x1) / (
                                                    y2 - y1
                                                )  # calculo la pendiente de la mano
                                                y = (
                                                    (0.40 - x1 + m * y1)
                                                ) / m - 0.01  # predice la posicipn y en la que acabara
                                        ...
                                        if x1 > x2:
                                            if y2 != y1:
                                                ...
                                                y = (
                                                    (0.40 - x2 + m * y2) / m
                                                ) - 0.01  # predice la posicion y en la que acabara.
                                        ...
                                        cory_mano = (
                                            arm_group.get_current_pose().pose.position.y
                                        )  # guarda la coordenada de la y del brazo
                                        corz_mano = (
                                            arm_group.get_current_pose().pose.position.z
                                        )  # guarda la coordenada de la z del brazo
                                        ...
                                        poseyf = round(
                                            posey, 2
                                        )  # zona a donde se mueve la pinza, se le resta la posicion de la pelota a la actual de la mano para alcanzarla
                                        posezf = round(posez, 2)

                                        wpose = (
                                            arm_group.get_current_pose().pose
                                        )  # guarda arm.group... donde luego se anadira la zona a la que se mover
                                        ...
                                        wpose = (
                                            arm_group.get_current_pose().pose
                                        )  # guarda en w.pose la posicion actual del brazo
                                        posicion_brazo_x.append(
                                            arm_group.get_current_pose().pose.position.x
                                        )
                                        posicion_brazo_y.append(
                                    ...
                                    while x_actual > corx_mano + 0.25:
                                        ...
                                        if x2 > x1:
                                            if (
                                                ...
                                                y = (
                                                    (0.40 - x1 + m * y1) / m
                                                ) - 0.01  # predice la posicipn y en la que acabara
                                        ...
                                        if x1 > x2:
                                            if y2 != y1:
                                                ...
                                                y = (
                                                    (0.40 - x2 + m * y2) / m
                                                ) - 0.018  # predice la posicion y en la que acabara.
                ...
                if x2 != x1:
                    if x_actual < 1.55:
                        ...
                        while x_actual > corx_mano + 0.25:
                            ...
                            if (
                                posexf > 0.5
                            ):  # NO LLEGA A LA PELOTA PORQUE NO ESTA EN SU ALCANCE
